...
public class AvailableBooksTransaction extends TransactionL{
	//GUI Components
	private String transactionErrorMessage = "";

	BookCollection myAvailableBooks;
	...
	//Constructor
	public AvailableBooksTransaction(Worker worker) throws Exception{
		super(worker);
	}
	...
	public Object getState(String key){
		...
		if(key.equals("BookList") == true){
			System.out.println("!NO! AvailableBooksTransaction.getState(...) " + key);
			System.exit(-1);
			return null;
		}
		if(key.equals("deep_props_vec_book_list")){
			return myAvailableBooks.getState("deep_props_vec");
		}
	...
	public void exportCsv(){
		CSVHandler csvh = new CSVHandler(new String[] {"Barcode", "Title", "Author1", "Discipline", "Status"}, 
				(Vector<Properties>)myAvailableBooks.getState("deep_props_vec"), 
				true, 
				true);
		String ReportsAvailableBooksDirectoryName = Librarian.REPORTS_DIRECTORY + File.separator + "Available_Books";
		File EOPReportsAvailableBooksDirectory = new File(ReportsAvailableBooksDirectoryName);
		if (EOPReportsAvailableBooksDirectory.exists() == false)
		{
			boolean EOPReportsAvailableBooksFlag = EOPReportsAvailableBooksDirectory.mkdirs();
		}
		exportStatusProps = new Properties();
		exportStatusProps.setProperty("FileName", ReportsAvailableBooksDirectoryName + File.separator + "available_books_" + csvh.getTodaysDateAndTime() + ".csv");
	...
	//TODO - process transaction method
	public void processTransaction(){
		myAvailableBooks = new BookCollection();
		myCollection = myAvailableBooks.findAvailableBooks();
		int numBooks = (Integer)myAvailableBooks.getState("BookCount");
	...
	protected ViewL createTransactionView() {
		ViewL localView = (ViewL)myViews.get("AvailableBooksView");

		if(localView == null){
			localView = ViewFactory.createView("AvailableBooksView", this);
