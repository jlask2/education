...
/**KnapSack Class: Uses dynamic programming strategy to find the optimal value that 
 * a knapsack can hold subject to the constraints of a maximum holding capacity and 
 * a set of weights with associated values*/
public class KnapSack{
	 ...
	 * Replace the ks[i][j] index with the max value*/
	      ...
	      for(int j = 0; j < W+1; j++){ 
	    	  ks[0][j] = 0;
	             for (int i = 1; i < n + 1; i++){ 
	                   for (int h = 0; h < W+1; h++){
	                         if ((h-w[i-1]) >= 0){
	                        	 ks[i][h] = max(ks[i-1][h], v[i-1] + ks[i-1][h-w[i-1]]); //either the cell above
	                        	 				//or the cell above and to the left wi positions plus the new vi 			
	                         }else{																						
	...
	/**max method: return the maximum of the two values given
	 * If equal, return value1*/
	...
	/**getOptimalSets method: returns a String containing the list of items that are included in the knapsack 
	 * This should be used only after the matrix ks is computed 
	 * To compute the optimal sets we start at the bottom right corner of the matrix
	 * (Where the optimal value is given)*/
	protected String getOptimalSets(){
		int i = ks.length - 1;      //the last row corresponding to the last item
		for(int j = ks[0].length-1; j >=0 && i > 0; i--){ //loop through by decrementing till 0 
			if(ks[i][j] != ks[i-1][j]){             // if the cell above is different
				optimalSets.add(i);				    // add the item/index to the optimal sets 
				j -= w[i-1];				//shift the column left by the weight of the previous item
			}
		}									//repeat until you get to the first row
		String os = "The optimal sets are: { ";
		for(int index = 0; index < optimalSets.size(); index++){
			os += optimalSets.get(index)+" "; //concatenate the sets to the string os
		}
		os += "}";
