<h1 class="page-header">Search For Over Due Tasks</h1>

<form role="form" id="searchoverduetasks-form" name="searchoverduetasks-form">
    <input type="hidden" name="FormID" value="search-over-due-tasks">
    <div class="row">
...
<script>
    ...
    $(function() {
        ...
        //=========================================================
	    // Recent Over Due Tasks Search
	    //=========================================================
        call_searchTenReports("task", "TaskID", "In Progress", "overDueTasks").done(function(result) {
            var results = JSON.parse(result);
            var status = results.status;
            
            if(status.indexOf("success") >= 0) {
	            $('#results-wrapper').empty();
	            $('#results-wrapper').append('<h3 class="no-results">Recent Over Due Tasks</h3>');
        ...
        //=========================================================
	    // End Recent Over Due Tasks Search
	    //=========================================================
        
        //=========================================================
        // Task Library Search Code
        //=========================================================
        $("#searchoverduetasks-form").submit(function(event) {
            var theForm = $(this);
            ...
            process_form_new(theForm, event).done(function(result) {
                ...
                if(status.indexOf("success") >= 0) {
                    ...
                    if(results.libraries.length <= 0) {
                        ...
                        searchString = "All Over Due Tasks";
                    } else if(($('#search-tasks-by-project-name').val() === "")
                              &&($('#project-cat').val() !== "Select a Category")&&($('#project-cat').val() !== "---------")) {
                        searchString = 'Over Due Tasks Matching Selected Category "' + $('#project-cat').val() + '"';
                    } else if(($('#search-tasks-by-project-name').val() !== "")
                              &&($('#project-cat').val() !== "Select a Category")&&($('#project-cat').val() !== "---------")) {
                        searchString = 'Over Due Tasks Matching Selected Category "' + $('#project-cat').val() + '" With Project Name Like "' + $('#search-tasks-by-project-name').val() + '"';
                    } else if(($('#search-tasks-by-project-name').val() !== "")
                              &&(($('#project-cat').val() === "Select a Category")||($('#project-cat').val() === "---------"))) {
                        searchString = 'Over Due Tasks Matching Project Name Like "' + $('#search-tasks-by-project-name').val() + '"';
                    } else {
                    }
