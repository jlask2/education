<?php
    ...
    switch($table_name) {
        ...
        case 'task' :
            ...
            $row_name_two = 'Name';
        break;
        // Special case for when we don't need the project, we just need the task itself
        case 'task_direct':
			$row_name = 'TaskID';
        break;
        case 'project' :
        	$row_name = 'ProjectCode';
        break;
    }
    ...
    if($table_name == 'task') {
        ...
        close_connection();
    } else if($table_name == 'task_direct') { 
	    $new_table_name = 'task';
	    if($searchStmt = $db->prepare("SELECT * FROM $new_table_name WHERE $row_name = $primary_key")) {
            if($searchStmt->execute()) {
                $result = $searchStmt->get_result();
                $rows['status'] = "success_QUERY";
                while($r = $result->fetch_assoc()) {
