<h1 class="page-header">Search For Late Tasks</h1>

<form role="form" id="searchlatetasks-form" name="searchlatetasks-form">
    <input type="hidden" name="FormID" value="search-late-tasks">
    <div class="row">
...
<script>
    ...
    $(function() {
        ...
        //=========================================================
	    // Recent Late Tasks Search
	    //=========================================================
        call_searchTenReports("task", "TaskID", "Complete", "lateTasks").done(function(result) {
            var results = JSON.parse(result);
            var status = results.status;
            
            if(status.indexOf("success") >= 0) {
	            $('#results-wrapper').empty();
	            $('#results-wrapper').append('<h3 class="no-results">Recent Late Tasks</h3>');
        ...
        //=========================================================
	    // End Recent Late Tasks Search
	    //=========================================================
        
        //=========================================================
        // Task Library Search Code
        //=========================================================
        $("#searchlatetasks-form").submit(function(event) {
            var theForm = $(this);
            ...
            process_form_new(theForm, event).done(function(result) {
                ...
                if(status.indexOf("success") >= 0) {
                    ...
                    if(results.libraries.length <= 0) {
                        ...
                        searchString = "All Late Tasks";
                    } else if(($('#search-tasks-by-project-name').val() === "")
                              &&($('#project-cat').val() !== "Select a Category")&&($('#project-cat').val() !== "---------")) {
                        searchString = 'Late Tasks Matching Selected Category "' + $('#project-cat').val() + '"';
                    } else if(($('#search-tasks-by-project-name').val() !== "")
                              &&($('#project-cat').val() !== "Select a Category")&&($('#project-cat').val() !== "---------")) {
                        searchString = 'Late Tasks Matching Selected Category "' + $('#project-cat').val() + '" With Project Name Like "' + $('#search-tasks-by-project-name').val() + '"';
                    } else if(($('#search-tasks-by-project-name').val() !== "")
                              &&(($('#project-cat').val() === "Select a Category")||($('#project-cat').val() === "---------"))) {
                        searchString = 'Late Tasks Matching Project Name Like "' + $('#search-tasks-by-project-name').val() + '"';
                    } else {
                    }
