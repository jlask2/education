...
(function( $ )
	...
	$.fn.sticky = function(note, options, callback) 
		...
		if(display)
            ...
            if(numNotes == 0) {
                $('#' + uniqID).addClass('first-in-queue');
                numNotes++;
            } else {
                numNotes++;
            }
		...
		$('.sticky').ready(function() {
			...
			if(settings['autoclose']) { 
                $('#' + uniqID).delay(settings['autoclose']).fadeOut(settings['speed'], function() {
                    decNotes();
                    if($('#' + uniqID).hasClass('first-in-queue')) {
                        $('#' + uniqID).prev('.sticky').addClass('first-in-queue');
                    }
                    $('#' + uniqID).remove();
                });
            }
		...
		$('.sticky-close').click(function() {
            var theObject = $(this).attr('rel');
            $('#' + theObject).dequeue().fadeOut(settings['speed']);
        });
