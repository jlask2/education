<h1 class="page-header">Search For Task Due Dates</h1>

<form role="form" id="searchtaskduedates-form" name="searchtaskduedates-form">
    <input type="hidden" name="FormID" value="search-task-due-dates">
    <div class="row">
...
<script>
    ...
    $(function() {
        ...
        //=========================================================
	    // Recent Task Due Dates Search
	    //=========================================================
        call_searchTenReports("task", "TaskID", "In Progress", "taskDueDates").done(function(result) {
            var results = JSON.parse(result);
            var status = results.status;
            
            if(status.indexOf("success") >= 0) {
	            $('#results-wrapper').empty();
	            $('#results-wrapper').append('<h3 class="no-results">Recent Task Due Dates</h3>');
        ...
        //=========================================================
	    // End Recent Task Due Dates Search
	    //=========================================================
        
        //=========================================================
        // Task Library Search Code
        //=========================================================
        $("#searchtaskduedates-form").submit(function(event) {
            var theForm = $(this);
            ...
            process_form_new(theForm, event).done(function(result) {
                ...
                if(status.indexOf("success") >= 0) {
                    ...
                    if(results.libraries.length <= 0) {
                        ...
                        searchString = "All Task Due Dates";
                    } else if(($('#search-tasks-by-project-name').val() === "")
                              &&($('#project-cat').val() !== "Select a Category")&&($('#project-cat').val() !== "---------")) {
                        searchString = 'Task Due Dates Matching Selected Category "' + $('#project-cat').val() + '"';
                    } else if(($('#search-tasks-by-project-name').val() !== "")
                              &&($('#project-cat').val() !== "Select a Category")&&($('#project-cat').val() !== "---------")) {
                        searchString = 'Task Due Dates Matching Selected Category "' + $('#project-cat').val() + '" With Project Name Like "' + $('#search-tasks-by-project-name').val() + '"';
                    } else if(($('#search-tasks-by-project-name').val() !== "")
                              &&(($('#project-cat').val() === "Select a Category")||($('#project-cat').val() === "---------"))) {
                        searchString = 'Task Due Dates Matching Project Name Like "' + $('#search-tasks-by-project-name').val() + '"';
                    } else {
                    }
