...
//			DEBUG:
	...
	/**
	 * Execute generic update query.
	 */
 	//------------------------------------------------------------
 	protected Integer executeUpdateQuery(String updateQueryString) throws SQLException
 	{
		try
		{
			// connect to the database
			...
			if (theDBConnection == null)
			{
				System.err.println("Persistable.executeUpdateQuery - Could not connect to database!");
				return null;
			}
			...
			{
				System.err.println("Persistable.executeUpdateQuery - Missing update SQL Statement!");
				return null;
			}
			...
			// The method executeUpdate executes a query on the database. The
			// return result is of type integer which indicates the number of rows updated
			int returnCode = theStatement.executeUpdate(updateQueryString);
...
//			DEBUG:
			System.err.println( "An SQL Error Occured:" + sqle + "\n" + sqle.getErrorCode() + "\n" + sqle.getMessage() + "\n" + sqle);
			new Event(Event.getLeafLevelClassName(this), "executeUpdateQuery", "SQL Exception: " + sqle.getErrorCode() + ": " + sqle.getMessage(), Event.ERROR);
			throw sqle;
		}
		finally
		{
