...
/**
 * UnionFind Class: Contains methods for applying a weighted union find with a
...
public class UnionFind {
	...
	/**
	 * findRoot method: Finds the parent of the given element and determines
	 * what set the element belongs to
	 */
	protected int findRoot(int x) {
		while (A[x] > 0) {
			final int temp = A[x] - 1;
			if (A[A[x] - 1] >= 0) {
	...
	/**
	 * union method: Joins two subsets together, joining the smaller set to the
	 * larger set. Breaks ties discriminately.
	 */
	protected void union(int x, int y) {
		if (A[x] < A[y] || A[x] == A[y]) {// if equal, join the set of y with
											// the set of x
			A[x] = A[x] + A[y];
			A[y] = x + 1;
		} else {
